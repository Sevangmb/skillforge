      ...
      subscribeWithSelector((set, get) => ({
        ...
        getAvailableSkills: () => {
          ...
          return skills.filter(skill => {
            ...
            const prereqsMet = !skill.prereqs || skill.prereqs.every(prereqId => 
        ...
        // Performance monitoring
        getPerformanceStats: () => {
          return statePerformanceMonitor.getPerformanceReport();
        },
      })),
      {
...
// Achievement hooks with memoization
export const useAchievements = () => useAppStore((state) => state.achievements);
export const useUnlockedAchievements = () => 
  useAppStore((state) => state.achievements.filter((a: Achievement) => (a as any).unlocked));
